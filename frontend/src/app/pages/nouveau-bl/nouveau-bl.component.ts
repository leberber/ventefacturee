...
type PaletteField = 'plastique' | 'bois';
...
export class NouveauBLComponent implements OnInit {
  ...
  dec(field: PaletteField) {
    const ctrl = this.form.get(field)!;
    const val = ctrl.value ?? 0;
    if (val > 0) ctrl.setValue(val - 1);
  }

  setQty(field: PaletteField, event: Event) {
  ...
  save() {
    ...
    this.expeditionsService.create(body).subscribe({
      next: exp => {
        this.messageService.add({ severity: 'success', summary: 'Succès', detail: `Expédition ${exp.bl_number} enregistrée`, life: 4000 });
