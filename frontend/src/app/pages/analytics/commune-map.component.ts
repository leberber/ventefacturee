...
export class CommuneMapComponent implements AfterViewInit, OnDestroy {
  ...
  private layerCode(layer: L.Layer): number | undefined {
    const v = (layer as any).feature?.properties?.['code'];
    return v != null ? Number(v) : undefined;
  }

  private layerName(layer: L.Layer): string | undefined {
    return (layer as any).feature?.properties?.['name'] as string | undefined;
  }
  ...
  private fmtCaisses(v: number): string {
  ...
  private buildCard(
    ...
    const header = `
      <div style="display:flex;align-items:flex-start;gap:9px;margin-bottom:12px">
        ${rank ? `<div style="min-width:28px;height:28px;border-radius:8px;background:${bc};
                    ...
                    font-size:9px;font-weight:800;color:#fff;flex-shrink:0;letter-spacing:-.3px">#${rank}</div>` : ''}
    ...
    if (!row || row.total === 0) return header +
      `<div style="color:#94a3b8;font-size:11px">Aucune vente sur la période</div>`;
    ...
    return header + `
      <div style="font-size:21px;font-weight:800;color:${bc};letter-spacing:-.5px;line-height:1;margin-bottom:10px">
        ${this.fmtCaisses(row.total)} <span style="font-size:11px;font-weight:500;color:#64748b">caisses</span></div>
      <div style="height:5px;background:#f1f5f9;border-radius:3px;overflow:hidden;margin-bottom:4px">
        <div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#93c5fd,${bc});
                    border-radius:3px;transition:width .35s ease"></div>
      </div>
  ...
  private placeTop1Pin(layer: L.Layer, name: string | undefined): void {
    ...
    const icon = L.divIcon({
      html: `<div style="
        ...
        box-shadow:0 2px 8px rgba(3,105,161,.45);
        pointer-events:none;letter-spacing:-.3px">#1</div>`,
  ...
  private animateFills(
    ...
    rest.forEach(({ path, fill }) => {
      path.style.transitionDelay = '0ms'; path.style.fill = fill;
    });
  ...
  private renderLayer(geo: GeoFeatureCollection, data: CommuneDatum[]): void {
    ...
    this.geoLayer = L.geoJSON(geo as any, {
        ...
        onEachFeature: (feature: any, layer: L.Layer) => {
          ...
          layer.on('mouseover', () => {
            ...
            const row  = code != null ? dataMap.get(code) : undefined;
            const rank = code != null ? rankMap.get(code) : undefined;
    ...
    const top1Layer = this.findLayerByCode(this.top1Code!);
    if (top1Layer) {
      const t = setTimeout(() => this.placeTop1Pin(top1Layer, this.layerName(top1Layer)), 80);
      this.cleanupTimers.push(t);
    }
  }
